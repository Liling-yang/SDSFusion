...
from model.fusion_module import MFSM
...
class DRBN(nn.Module):
    def __init__(self, args):
        ...
        self.MFSM = MFSM()
        self.MFSM.load_state_dict(torch.load('/mnt/data2/yll/low_light_enhancement/SDSFusion/pretrain/fusion/fusion_epoch_150.pth'))
        self.MFSM.eval()
        self.up_v1 = nn.Conv2d(8, 16, kernel_size=1)
        self.down_ch_v1 = nn.Conv2d(32, 16, kernel_size=1)
        self.down_ch_v2 = nn.Conv2d(32, 16, kernel_size=1)
        self.down_ch_v4 = nn.Conv2d(64, 32, kernel_size=1)

    def get_MFSM_features(self, v):
        v_1 = self.MFSM.V_en_1(v)
        v_2 = self.MFSM.V_en_2(self.MFSM.V_down1(v_1))
        v_3 = self.MFSM.V_en_3(self.MFSM.V_down2(v_2))
        return v_1, v_2, v_3
        # return gamma_correction(v_1, 0.5), gamma_correction(v_2, 0.5), gamma_correction(v_3, 0.5)
    ...
    def forward_1(self, x_input): # 计算前三个recurrence的Res和feature
        x = x_input
        with torch.no_grad():
            v_1, v_2, v_3 = self.get_MFSM_features(x)

        res_g1_s1, res_g1_s2, res_g1_s4, feat_g1_s1, feat_g1_s2, feat_g1_s4 = self.recur1([0, torch.cat((x, x), 1), 0, 0, 0, 0, 0, 0])
        
        v_1 = self.up_v1(v_1)
        feat_g1_s1 = torch.cat((feat_g1_s1, v_1), dim=1)
        feat_g1_s2 = torch.cat((feat_g1_s2, v_2), dim=1)
        feat_g1_s4 = torch.cat((feat_g1_s4, v_3), dim=1)
        feat_g1_s1 = self.down_ch_v1(feat_g1_s1)
        feat_g1_s2 = self.down_ch_v2(feat_g1_s2)
        feat_g1_s4 = self.down_ch_v4(feat_g1_s4)
    ...
    def forward_2(self, x, res_g3_s1, res_g3_s2, res_g3_s4, feat_g3_s1, feat_g3_s2, feat_g3_s4, seg_orin, seg_fea): # test
        # v_1 = self.up_v1(v_1)
        # feat_g3_s1 = torch.cat((feat_g3_s1, v_1), dim=1)
        # feat_g3_s2 = torch.cat((feat_g3_s2, v_2), dim=1)
        # feat_g3_s4 = torch.cat((feat_g3_s4, v_3), dim=1)
        # feat_g3_s1 = self.down_ch_v1(feat_g3_s1)
        # feat_g3_s2 = self.down_ch_v2(feat_g3_s2)
        # feat_g3_s4 = self.down_ch_v4(feat_g3_s4)
...
class DRBN_BU(nn.Module):
    ...
    def part_forward(self, x,seg_orin, seg_fea):
        ...
        elif seg_orin is not None: # 语义信息嵌入
...
def gamma_correction(features, gamma):
    max_val = features.max()
    min_val = features.min()
    features_normalized = (features - min_val) / (max_val - min_val)
    inv_gamma = 1.0 / gamma
    features_corrected = torch.pow(features_normalized, inv_gamma)
    features_corrected = features_corrected * (max_val - min_val) + min_val
