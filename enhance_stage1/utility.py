...
class checkpoint():
    def __init__(self, args):
        ...
        if args.load == '.':
            if os.path.basename(path1) == 'SDSFusion':
                self.dir = os.path.join(path1,'enhance_stage1','experiment',args.save)
            elif os.path.basename(path1) == 'enhance_stage1':
                self.dir = os.path.join(path1,'experiment',args.save)
        else:
            if os.path.basename(path1) == 'SDSFusion':
                self.dir = os.path.join(path1,'enhance_stage1','experiment',args.load)
            elif os.path.basename(path1) == 'enhance_stage1':
    ...
    def save_unpaired_results(self, filename, save_list, original_size):
        if os.path.basename(os.getcwd()) == 'SDSFusion':
            filename1 = os.path.join(os.getcwd(),'datasets/test/LLVIP/vi_en-s1/{}'.format(filename))
        elif os.path.basename(os.getcwd()) == 'enhance_stage1':
            path1 = os.path.dirname(os.getcwd())
            filename1 = os.path.join(path1,'datasets/test/LLVIP/vi_en-s1/{}'.format(filename))

        postfix = ('LR', 'HR_Pred', 'LR_NF_Pred')
        for v, p in zip(save_list, postfix):
            cropped = v[:, :, :original_size[0], :original_size[1]]
            normalized = cropped[0].data.mul(255 / self.args.rgb_range)
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
            if p == 'HR_Pred':
                misc.imsave('{}.jpg'.format(filename1,p), ndarr)
