...
class LRData(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Adjust the shape of lr to [3, h, w]
        if lr.ndim == 4 and lr.shape[0] == 1:
            lr = np.squeeze(lr, axis=0)  # Remove the first dimension if it's 1
        elif lr.ndim != 3:
            raise ValueError(f"Expected 3-dimensional input, but got {lr.ndim}-dimensional input")
